...
Partial Class MEDICAL_Login
    ...
    Protected Sub Button1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        cnn.Open()
        cmd = New SqlCommand("SELECT * FROM Medical_Store_Staff_Register WHERE Username='" & txtuname.Text & "' AND Password='" & txtpass.Text & "'", cnn)
        dr = cmd.ExecuteReader
        If (dr.Read) Then
            If (dr(0).ToString = txtuname.Text) Then
                MsgBox("Login Successfully", MsgBoxStyle.Information, "Login")
                Response.Redirect("MM1.aspx")
            End If
        Else
